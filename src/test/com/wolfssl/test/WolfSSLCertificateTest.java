                ...
                if (type == WolfSSL.ASN_OTHER_TYPE) {
                    ...
                    if (san.size() >= 3) {
                        ...
                        if (MS_UPN_OID.equals(oid) && valueBytes.length > 2) {
                            /* ASN.1 UTF8String starts with tag 0x0C and
                             * length. Skip first 2 bytes (tag + length)
                             * for simple case. Real implementation should
                             * use proper ASN.1 parser.
                             */
                            byte[] strBytes = new byte[valueBytes.length - 2];
                            System.arraycopy(valueBytes, 2, strBytes, 0,
                                strBytes.length);
                            String upn = new String(strBytes, "UTF-8");
                            assertNotNull("UPN should not be null", upn);
                        }
                    }
                }
            }
